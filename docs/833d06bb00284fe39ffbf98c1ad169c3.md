...
## 설치유무 및 버전확인  
OpenSilver 프로젝트를 시작하기 전에 .NET SDK와 OpenSilver 템플릿 설치 여부를 확인할 수 있습니다. 아래 명령어를 통해 설치 상태를 점검합니다.
...
Windows 환경에서는 CMD 창을 열고 위 명령어를 실행해 .NET SDK 버전을 확인할 수 있습니다.
...
macOS 또는 Linux 환경에서는 Terminal을 열고 위 명령어를 실행해 .NET SDK와 OpenSilver 템플릿 설치 여부를 점검할 수 있습니다.

설치가 제대로 되지 않은 경우, [Setting Up OpenSilver Tools](https://opensilver.net/tools) 가이드를 참고하여 필요한 환경을 구성할 수 있습니다.
