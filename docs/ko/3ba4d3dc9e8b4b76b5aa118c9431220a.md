...
## 프로젝트 생성  
터미널을 열고 원하는 프로젝트 폴더로 이동한 뒤, OpenSilver CLI 템플릿을 설치합니다. 다음 명령어를 실행할 수 있습니다.  
...
설치 후 새 프로젝트를 생성하려면 아래 명령어를 사용합니다.  
```
...
dotnet run
```
C# Dev Kit를 설치하면 디버깅 환경이 향상되며, 디버그 패널에서 실행 버튼을 클릭해 브라우저에서 애플리케이션을 확인할 수 있습니다. C# Dev Kit 없이도 실행 가능하며, 알림에 따라 자산을 추가하면 디버깅이 가능합니다.

## 추가 설정  
Visual Studio Code 확장은 [https://opensilver.net/download](https://opensilver.net/download)에서 다운로드할 수 있습니다. 이 확장은 XAML UI 디자이너를 제공하며, `opensilver.newApplication`과 같은 명령으로 새 프로젝트를 생성할 수 있습니다. XAML 파일을 시각적으로 편집하고, `opensilver.compileSolution`로 빌드할 수 있습니다.
