...
_Visual Studio에서 상단 opensilverapp1.browser 디버그 버튼 클릭 모습_
...
Visual Studio 상단의 "opensilverapp1.browser" 디버그 버튼을 클릭하거나 F5(디버그 모드), Ctrl+F5(디버그 없이 실행)를 눌러 프로젝트를 실행할 수 있습니다.
...
실행 결과, Microsoft Edge 브라우저에서 OpenSilver 애플리케이션이 표시됩니다. 기본적으로 "Hello, OpenSilver!" 텍스트와 버튼, 체크박스가 포함된 UI가 나타나며, 버튼 클릭 시 동작을 테스트할 수 있습니다. UI가 정상적으로 작동하는지 확인하고, 대화 상자는 제외한 UI만 점검할 수 있습니다.
...
"OpenSilverApp1" 이름으로 OpenSilver 애플리케이션 프로젝트를 생성하면 다음과 같이 3개의 프로젝트로 구성됩니다. "OpenSilverApp1"은 오픈실버의 메인 클래스 라이브러리 프로젝트로, MainPage.xaml과 App.xaml 등 OpenSilver의 기본 구조가 포함되어 있습니다. "OpenSilverApp1.Browser"는 오픈실버를 실행하는 WebAssembly 프로젝트로, Blazor WebAssembly를 기반으로 동작합니다. "OpenSilverApp1.Simulator"는 Windows 기반에서만 제공되는 데스크톱 기반의 오픈실버 디버깅을 위한 시뮬레이터입니다. 이 시뮬레이터는 Windows에서만 실행 가능하며, macOS, Linux, Raspberry Pi에서는 지원되지 않습니다.
...
명령 팔레트에서 "빌드" 명령을 선택하거나 단축키(Ctrl+Shift+B)를 사용해 빌드를 시작할 수 있습니다. "OpenSilverApp1.Browser" 프로젝트의 "dotnet build" 옵션을 선택하면 빌드가 진행되며, 터미널 창에서 결과를 확인할 수 있습니다.

## 실행  
Visual Studio Code에서 C# Dev Kit 확장을 설치한 경우, F5를 눌러 디버그를 시작할 수 있습니다. 시작 프로젝트를 선택하라는 메시지가 나타납니다.
...
실행 결과, Chrome 브라우저에서 OpenSilver 애플리케이션이 표시되며, 기본 URL은 `http://localhost:55592`입니다. 화면에는 "Hello, OpenSilver!" 텍스트와 버튼, 체크박스가 포함된 UI가 나타납니다. 버튼을 클릭하거나 체크박스를 선택해 UI 동작을 테스트할 수 있으며, 대화 상자는 제외하고 UI만 점검할 수 있습니다.