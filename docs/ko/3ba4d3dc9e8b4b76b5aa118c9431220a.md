...
**OpenSilverApp1**은 메인 클래스 라이브러리 프로젝트로, MainPage.xaml과 App.xaml 등 OpenSilver의 기본 구조를 포함합니다. **OpenSilverApp1.Browser**는 Blazor WebAssembly 기반의 WebAssembly 프로젝트로, 오픈실버를 브라우저에서 실행합니다. **OpenSilverApp1.Simulator**는 Windows에서만 제공되는 데스크톱 기반 디버깅 시뮬레이터로, macOS, Linux, Raspberry Pi에서는 지원되지 않습니다.
