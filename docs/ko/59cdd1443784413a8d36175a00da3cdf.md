...
# Command 사용  
이 섹션에서는 Button의 Command 속성을 활용하여, 커맨드 패턴을 적용하는 방법에 대해 설명합니다.  
이번 예제에서는 별도의 RelayCommand나 INotifyPropertyChanged 구현 없이, **RoutedCommand**를 활용합니다.  
또한 텍스트박스 입력값을 기반으로 CanExecute 로직을 구성하여, 텍스트가 입력되어야만 버튼이 활성화되도록 합니다.
