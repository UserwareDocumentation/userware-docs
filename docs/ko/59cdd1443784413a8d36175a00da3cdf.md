...
| Property (Class)          | Remark                                  | Type                  |
|---------------------------|-----------------------------------------|-----------------------|
| Content (ContentControl)  | 버튼에 표시할 내용을 정의                | object                |
| Command (ButtonBase)      | 명령 실행을 위한 바인딩                  | ICommand              |
| Click (ButtonBase)        | 버튼 클릭 시 발생하는 라우트 이벤트       | RoutedEventHandler    |
| Template (Control)        | 버튼의 외관을 커스터마이징하는 템플릿      | ControlTemplate       |
| ItemTemplate (ContentControl) | ContentPresenter의 데이터 템플릿      | DataTemplate          |
| IsEnabled (Control)       | 버튼의 활성화/비활성화 상태를 제어         | bool                  |
